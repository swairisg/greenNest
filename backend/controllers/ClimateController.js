...
//api: abcf79bc-863f-11f0-a59f-0242ac130006-abcf7a2a-863f-11f0-a59f-0242ac130006
...
const getAllClimateRecords = async (req, res, next) => {
  try {
    ...
    const records = await ClimateRecord.find()
      .sort({ createdAt: -1 })
      .limit(limit); //fetch all data sorted by createdAt in descending order

    if (!records || records.length === 0) {
      return res.status(404).json({ message: "No climate data found" }); //no data found
    }
...
const getLatestData = async (req, res, next) => {
  try {
    const latestRecord = await ClimateRecord.findOne().sort({ createdAt: -1 }); // Get most recent record
